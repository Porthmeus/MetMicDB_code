...
if (isset($_GET['value'])) {
        $classvalue = $_GET['value'];

        $all_order = $db->query("SELECT DISTINCT tax_order FROM taxonomy_generalstats WHERE Class='$classvalue' ORDER BY tax_order ASC") or die($db->error);
        
$current_letter = '';
if (mysqli_num_rows($all_order) !==0) {
                echo "<br>";
                while ($row = $all_order ->fetch_assoc()) {
                        $order = $row['tax_order'];
                        $first_letter = strtoupper(substr($order, 0, 1));
                        ...
                        echo "<a href=\"species.php?value=$order\">$order</a><br>";
