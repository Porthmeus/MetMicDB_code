...
$all_order = $db->query("SELECT DISTINCT tax_order FROM host_taxonomy ORDER BY tax_order ASC") or die($db->error);
...
if (mysqli_num_rows($all_order) !== 0) {
    ...
    while ($row = $all_order->fetch_assoc()) {
        ...
        if ($tax_order !== "NA") {
            $first_letter = strtoupper(substr($tax_order, 0, 1));
            ...
            echo "<a href=\"menuhosts.php?value=$tax_order\">$tax_order</a><br>";
        }
    }
} else {
