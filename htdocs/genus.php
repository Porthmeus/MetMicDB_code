...
if (isset($_GET['value'])) {
      $familyvalue = $_GET['value'];

        $all_genus = $db->query("SELECT DISTINCT Genus FROM taxonomy_generalstats WHERE Family='$familyvalue' ORDER BY Genus ASC") or die($db->error);
        
$current_letter = '';
if (mysqli_num_rows($all_genus) !==0) {
                echo "<br>";
                while ($row = $all_genus ->fetch_assoc()) {
                        $genus = $row['Genus'];
                        $first_letter = strtoupper(substr($genus, 0, 1));
                        ...
                        echo "<a href=\"species.php?value=$genus\">$genus</a><br>";
